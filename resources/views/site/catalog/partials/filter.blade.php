<select name="price" class="form-control d-inline w-25 me-4" title="Цена">
    ...
    <option value="min"@if (request()->price == 'min') selected @endif>{{ __('Дешевые товары') }}</option>
    <option value="max"@if (request()->price == 'max') selected @endif>{{ __('Дорогие товары') }}</option>
</select>
<!-- новинка -->
